document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".btn-mas").forEach((boton) => {
    boton.addEventListener("click", function () {
      const input = this.closest(".grupo-cant").querySelector(".cant");
  ...
  const botonConfirmar = document.querySelector('a[href="gracias.html"]');
  ...
  if (botonConfirmar) {
    botonConfirmar.addEventListener("click", (e) => {
      ...
      if (carrito.length === 0) {
        ...
        mensaje.innerHTML =
          "⚠️ <strong>El carrito está vacío.</strong> Agrega productos antes de continuar.";

        botonConfirmar.parentNode.insertAdjacentElement("afterend", mensaje);
  ...
  document.querySelectorAll(".btn-agregar").forEach((boton) => {
    boton.addEventListener("click", function () {
      ...
      const cantidad = parseInt(card.querySelector(".cant").value);
      ...
      this.innerHTML = '<i class="bi bi-check2-circle"></i> ¡Agregado!';
  ...
  function mostrarCarrito() {
    ...
    if (carrito.length === 0) {
      document.getElementById("tabla-carrito").classList.add("d-none");
